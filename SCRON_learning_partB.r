...
# locations of sub-sampling for the purchase stage locations_subsampling_purchase.csv and the rejection sites stored in reject_sites_stagename.txt
...
reject = read.table("reject_sites_open.txt");
reject = as.vector(reject[,1]);

reject2 = read.table("reject_sites_click.txt");
reject2 = as.vector(reject2[,1]);

reject3 = read.table("reject_sites_purchase.txt");
...
finalMatrix <- foreach(rr=1:part_split, .combine='+') %dopar% {
...
covar_all_est_c = covar_all[,-reject2];  covar_all_est_p = covar_all[,-reject3];

covar_all = covar_all[,-reject];
...
covar_all_c = covar_all_c[,-reject2];
...
covar_all_p = covar_all_p[,-reject3];
...
scales_rand = c(0.005, 0.005, 0.001);
...
total_output = rep(0, length(beta_o)+length(betas_o)+length(beta_c)+length(betas_c)+length(beta_p)+length(betas_p)+
                     length(sig_rand));
...
for(ll in 1:mcmc_run){
  ...
  param1 = w_param1+dim(beta_rand)[1];  
...
write.table(finalMatrix, "SCRON_learning_output.txt", col.names=F, row.names=F, quote=F);

save(finalMatrix, file = "SCRON_learning_output.rda")
