...
update_beta_x_tmcmc_model2_sp_parallel <- function(method, S_part, N, len_x, XX, covar_x, covars_x, beta_x, 
                                          betas_x, rand, eps, scales_r, scalespike, deltaparam, paramslab, paramspike, mean_betaxx, sig_betaxx, prob_x){
  
  betax = append(beta_x, betas_x);
  
  mean_betax = rep(0, length(betax));  sig_betax = rep(0, length(betax));
  
  pos1 = which(deltaparam==1); 
  pos2 = which(deltaparam==0);
  pos3 = which(is.na(deltaparam)==T);
  
  if(length(pos1)!=0){
    mean_betax[pos1] = paramslab[pos1,1];
    sig_betax[pos1] = paramslab[pos1,2];
  }
  if(length(pos2)!=0){
    mean_betax[pos2] = paramspike[pos2,1];
    sig_betax[pos2] = paramspike[pos2,2];
  }
  
  mean_betax[pos3] = mean_betaxx[pos3];
  sig_betax[pos3] = sig_betaxx[pos3];
  ...
  if(method=="tmcmc"){
    ...
    change_ep = rep(0, length(betax));
    change_ep = rnorm(n=1, mean=0, sd=1);
    change_ep = abs(change_ep);
    
    z_change = rep(0, length(change_ep));
    z_change = rbinom(n=length(betax), size=1, prob=0.5);
    z_change[z_change==0] = -1;
    
    betax_new[pos1] = betax[pos1] + (scales_r[pos1]*z_change[pos1]*change_ep);
    
    #betax_new[pos2] = betax[pos2] + (scalespike*z_change[pos2]*change_ep);
    betax_new[pos2] = betax[pos2] + (scales_r[pos2]*z_change[pos2]*change_ep);
    
    betax_new[pos3] = betax[pos3] + (scales_r[pos3]*z_change[pos3]*change_ep);
    
  }
  
  beta_x_new = betax_new[1:length(beta_x)];
  betas_x_new = betax_new[(length(beta_x)+1):length(betax)];
  
  prob_x_new = logit_calc_spline_rand(N, beta_x_new, betas_x_new, rand, covar_x, covars_x, eps);
  pos3 = which(prob_x_new<eps);
  prob_x_new[pos3] = prob_x_new[pos3] + eps;
...
update_beta_x_tmcmc_model2_sp <- function(method, N, len_x, XX, covar_x, covars_x, beta_x, 
                                          betas_x, rand, eps, scales_r, scalespike, deltaparam, paramslab, paramspike, mean_betaxx, sig_betaxx, prob_x){
...
update_delta_sp <- function(pos_spike, pi, beta_x, betas_x, paramslab, paramspike){
  
  betax = append(beta_x, betas_x);
  
  density1 = dnorm(betax[pos_spike], mean=paramslab[pos_spike,1], sd=paramslab[pos_spike,2]);
  
  density2 = dnorm(betax[pos_spike], mean=paramspike[pos_spike,1], sd=paramspike[pos_spike,2]);
  
  ratio = density2/density1;
  
  deno = 1+(((1-pi)/pi)*ratio);
  
  prob = 1/deno;
  
  delta = rbinom(n=length(pos_spike), size=1, prob=prob);
  
  return(delta);
  
}

update_delta_sp_parallel <- function(part_split, pos_spike, pi, beta_x, betas_x, paramslab, paramspike){
  ...
  ratio = (density2/density1)^(1/part_split);
  
  deno = 1+((((1-pi)/pi)^(1/part_split))*ratio);
...
sigma_beta_sp_parallel <- function(S_part, pos_spike, beta_x, betas_x, a_sigma, b_sigma, delta, rfactor){
  
  betax = append(beta_x, betas_x);
  len_full = length(betax);
  
  r_assign = rep(rfactor, len_full);
  pos = which(delta==1);
  if(length(pos)!=0){
    r_assign[pos] = 1;
  }
  
  param1 = ((a_sigma-1)/S_part) + 1 + (1/2);
  
  param2 =  (b_sigma/S_part) + ((1/(2*S_part))*betax[pos_spike]^2/r_assign[pos_spike]);
  
  sig = rgamma(n=length(pos_spike), shape=param1, rate=param2);
...
sigma_beta_sp <- function(pos_spike, beta_x, betas_x, a_sigma, b_sigma, delta, rfactor){
  ...
  param2 =  b_sigma + ((1/2)*betax[pos_spike]^2/r_assign[pos_spike]);
