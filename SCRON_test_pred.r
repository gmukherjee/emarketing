setwd("C:\\Users\\wkar\\Downloads\\EmailPaper_SWG_Final\\TrialFinal\\SCRON")
cojoin <- function(X, Y){
...
first_mcmc = read.table("SCRON_learning_output.txt");
...
write.table(finalMatrix, "SCRON_test_pred.txt", col.names=T, row.names=F, quote=F);

#write.table(finalMatrix, "results_parallel/model6_test_model3_parallel_putaside.txt", col.names=T, row.names=F, quote=F);
