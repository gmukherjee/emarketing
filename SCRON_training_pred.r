...
finalMatrix <- foreach(rr=1:part_split, .combine='rbind', .options.snow = opts) %dopar% {
  ...
  for(ll in 1:(mcmc_run-burn)){
    ...
    prob_c_con = (covar_all_est_c%*%(beta_c)) + (train_c%*%(betas_c));
    
    prob_p_con = (covar_all_est_p%*%(beta_p)) + (train_p%*%(betas_p));
...
write.table(finalMatrix, "SCRON_training_pred.txt", col.names=T, row.names=F, quote=F);
