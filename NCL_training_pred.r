...
locs_purchase = read.csv("results_save2/locations_subsampling_purchase.csv", T);

# Training Set
total_data = read.table("../data/training_test/training_set2_type.txt", T);
...
########### for open stage ############
basics <- read_excel("results_save1/coefficient_model52.xlsx",sheet=1);
...
########### for click stage ############
basics <- read_excel("results_save1/coefficient_model5_click.xlsx",sheet=1);
...
####### for purchase stage ###########
basics <- read_excel("results_save1/coefficient_model5_purchase.xlsx",sheet=1);
...
first_mcmc = read.table("results_parallel/SCRON_parallel_beta_sigma2.txt");
...
finalMatrix <- foreach(rr=1:part_split, .combine='rbind', .options.snow = opts) %dopar% {
  ...
  mcmc_run = 10; ##### number of MCMC run
  burn = 0; ###### number of burn-in 
...
write.table(finalMatrix, "results_parallel/SCRON_training_model2_pred.txt", col.names=T, row.names=F, quote=F);
