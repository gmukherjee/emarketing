...
finalMatrix <- foreach(rr=1:part_split, .combine='+', .options.snow = opts) %dopar% {
  ...
  ##### for open stage #############
  open_covars_o = covar_b.spline_model3(trans="log", degree, data$daysSincePurchased, data$daysSinceOpened);
  
  ######## for click stage ###########
  click_covars_o = covar_b.spline_model3(trans="log", degree, data_c$daysSincePurchased, data_c$daysSinceClicked);
  
  ########## for purchase stage ###########
  purchase_covars_o = covar_b.spline_model3_purchase(trans="log", degree, data_p$daysSincePurchased);
  
  ###################### structures for producing training probs ##############
  train_c = covar_b.spline_model3(trans="log", degree, data$daysSincePurchased, data$daysSinceClicked);
  
  train_p = covar_b.spline_model3_purchase(trans="log", degree, data$daysSincePurchased);
  ...
  ########### for open stage ############
  basics <- read_excel("initial_coeff_NCG_open.xlsx",sheet=1);
  ...
  ########### for click stage ############
  basics <- read_excel("initial_coeff_NCG_click.xlsx",sheet=1);
  ...
  ####### for purchase stage ###########
  basics <- read_excel("initial_coeff_NCG_purchase.xlsx",sheet=1);
...
write.table(finalMatrix, "NCG_learning_output.txt", col.names=F, row.names=F, quote=F);
save(finalMatrix, file = "NCG_learning_output.rda");
